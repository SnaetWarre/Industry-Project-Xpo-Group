// Inject general and FFD-specific CSS if not already present
(function injectFfdCss() {
  if (document.getElementById('ffd-bot-css')) return;
  const style = document.createElement('style');
  style.id = 'ffd-bot-css';
  style.textContent = `
    :root {
      --ffd-color: #DB6245;
      --chat-width: 350px;
    ...
    .chatbot-container[data-website="ffd"] .bot-message-container img {
      filter: brightness(0) saturate(100%) invert(45%) sepia(95%) saturate(385%) hue-rotate(327deg) brightness(91%) contrast(92%);
    }
    ...
    .chatbot-container[data-website="ffd"] .register-main-btn { background: var(--ffd-color); }
    ...
    .chatbot-container[data-website="ffd"] .register-button.subtle { color: var(--ffd-color); }
    ...
    .chatbot-container[data-website="ffd"] .sticky-register-btn { background: var(--ffd-color); }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // FFD bot configuration
  const botName = 'FLORBot';
  const welcomeMessage = 'Hoi! Ik ben je beursassistent voor de vloeren- en isolatiebeurs. Ik kan je helpen met informatie over vloeren, isolatie en gerelateerde producten. Waar ben je naar op zoek?';
  const websiteId = 'ffd';
  ...
  registerBtn.onclick = async () => {
    ...
    let url = 'https://ffd25.registration.xpogroup.com/invitation';
