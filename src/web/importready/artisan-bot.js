...
.chatbot-container[data-website='artisan'] .sticky-register-btn {
  background: var(--artisan-color);
}
`;

function injectArtisanCss() {
  if (!document.getElementById('artisan-bot-css')) {
    const style = document.createElement('style');
    style.id = 'artisan-bot-css';
    style.textContent = ARTISAN_CSS;
...
function initArtisanBot() {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    chatbotContainer.dataset.website = 'artisan';
    ...
    registerBtn.onclick = async () => {
      ...
      await trackAnalyticsEvent('registration', { sessionId, website: 'artisan', profileInfo });
      window.open('https://artisan25visitors.registration.xpogroup.com/?language=nl', '_blank');
    };
    // Disclaimer close
    if (disclaimerClose) {
    ...
    form.onsubmit = async (e) => {
      ...
      if (registrationStep) {
        ...
        await trackAnalyticsEvent('chat_start', { sessionId: sid, website: 'artisan', profileInfo: text });
      ...
      try {
        const response = await fetch(`${API_URL}/api/chat`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ query: text, website: 'artisan', sessionId }),
        });
