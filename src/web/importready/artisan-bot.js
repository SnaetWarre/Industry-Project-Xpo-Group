// Inject general and Artisan-specific CSS if not already present
(function injectArtisanCss() {
  if (document.getElementById('artisan-bot-css')) return;
  const style = document.createElement('style');
  style.id = 'artisan-bot-css';
  style.textContent = `
    :root {
    ...
    .chatbot-container[data-website="artisan"] .bot-message-container img {
      filter: brightness(0) saturate(100%) invert(23%) sepia(0%) saturate(0%) hue-rotate(241deg) brightness(98%) contrast(84%);
    }
    ...
    .chatbot-container[data-website="artisan"] .register-main-btn { background: var(--artisan-color); }
    ...
    .chatbot-container[data-website="artisan"] .register-button.subtle { color: var(--artisan-color); }
    ...
    .chatbot-container[data-website="artisan"] .sticky-register-btn { background: var(--artisan-color); }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Artisan bot configuration
  const botName = 'ArtisanBot';
  const welcomeMessage = 'Hoi! Ik ben je beursassistent voor artisanale en ambachtelijke producten. Ik kan je helpen met informatie over voedsel, dranken en andere ambachtelijke producten. Waar ben je naar op zoek?';
  const websiteId = 'artisan';
  ...
  // Insert the button immediately after the headerTitle (ArtisanBot text)
  ...
  registerBtn.onclick = async () => {
    ...
    let url = 'https://www.artisan-xpo.be/nl/plan-uw-bezoek/registreer-uw-bezoek/';
