...
class EventChatbot:
    ...
    def _get_contextual_snippet(self, text: str, keyword: str, window_size: int = 10000) -> Optional[str]: 
    ...
    def get_relevant_events(self, user_input: str, extracted_keywords: List[str]) -> List[Dict]:
        if not self.vector_client.events:
        ...
        if not extracted_keywords and self.vector_client.data_path != "ffd_site_data_cleaned.json":
            print(f"[DEBUG get_relevant_events] No specific keywords, using all {len(self.vector_client.events)} items from {self.vector_client.data_path} as context.")
            return self.vector_client.events # Return all loaded events for the specific event file
            ...
            return self.vector_client.events[:5] # Return first 5 as a sample if no keywords found

        print(f"[DEBUG get_relevant_events] Searching with keywords: {extracted_keywords}")
        
        scored_events = []

        for i, event in enumerate(self.vector_client.events):
            score = 0
...
User's Question: {user_input}"""
    ...
    def _add_stand_numbers_to_events(self):
        ...
        for evt in self.vector_client.events:
